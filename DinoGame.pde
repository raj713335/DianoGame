...
void writeInfo() {
  ...
  if (showBestEachGen) { //if showing the best for each gen then write the applicable info
    ...
    if (genPlayerTemp.score>highest){
      highest=genPlayerTemp.score;
      
    }
    text("Highest Score : " + highest, 500, height - 30);
    text("raj713335", 50, 70);
    ...
    //text(, width/2-180, height-30);
    if (floor(pop.populationLife/3.0)>highest){
      highest=floor(pop.populationLife/3.0);
